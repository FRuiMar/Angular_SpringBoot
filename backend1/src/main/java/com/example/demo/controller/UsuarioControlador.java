...
public class UsuarioControlador {
	...
	// @PostMapping(path = "/addUsuario", consumes =
	// MediaType.APPLICATION_JSON_VALUE)
	// public void addUsuario(@RequestBody DatosCreacionUsuario u,
	...
	// Membresia membresia = memRep.findById(u.membresia);
	//
	// usuRep.save(new Usuario(u.id, u.dni, u.nombre, u.apellido, u.email,
	// hashedPassword, rol, u.imagen, membresia));
	//
	// }

	@PostMapping(path = "/addUsuario", consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
	public DTO addUsuario(@RequestPart("datosCreacionUsuario") DatosCreacionUsuario u,
		...
		try {
			...
			String filename = timestamp + "-" + imagen.getOriginalFilename();
			String uploadDir = "assets/usuarios/";
			Files.createDirectories(Paths.get(uploadDir));
			Files.write(Paths.get(uploadDir, filename), imagen.getBytes());
			System.out.println("before save");

			// Crear y guardar el nuevo usuario
			Usuario nuevoUsuario = new Usuario(0, u.dni, u.nombre, u.apellido, u.email, hashedPassword, rol,
					"/assets/usuarios/" + filename, membresia);
